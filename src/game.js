...
import {Pawn, Rook, Knight, Bishop, Queen, King, isCheck, getLegalMoves} from './pieces.js'
    ...
    if(moveType === 'queen') {
      ...
      for(let i = 0; i < state.squares.length; i++) {
        for(let j = 0; j < state.squares[i].length; j++) {
          ...
          if(piece && piece.color === turnPlayer && getLegalMoves([i, j], piece, board).length > 0)
            legalMoves = true;
        }
      }
...
function Square(props) {
  return (
    <div
      ...
      {props.piece ? props.piece.render(props.turnPlayer === props.piece.color) : null}
    </div>
  );
}
...
function Coordinate(props) {
  return (
    <button
      className={"coordinate " + props.alignment}
      key={props.label}
      >
        {props.label}
      </button>
...
function QueenQuery(props) {
  var piece = (C) => new C({color: props.color, id: 0, onclick: ((ev) => {alert("yolo")})});
  return (
    <div
      id="queen_query"
      className="queen_query"
      >
...
// console.log(sourceCol + " : " + sourceRow)
    ...
    if(sourcePiece.name === 'pawn') {
      if(moveType === 'march') {
        ...
        squares[targetCol][targetRow] = new Queen({color: sourcePiece.color,id: 'id',board: this});
      }
      ...
      } else if(sourcePiece.name === 'rook' && castling[sourcePiece.color][sourcePiece.num] === 1) {
    ...
    if(targetPiece  && targetPiece.name === 'rook' && castling[targetPiece.color].includes(1)) {
  ...
  render() {
      ...
      for(let j = 0; j < this.state.squares[i].length; j++) {
        ...
        if(kingpos === chessCoord) {
          if(this.state.status === 'check')
            color = 'orange'
          else if(this.state.status === 'checkmate')
            color = 'red'
        ...
        columns.push(this.renderSquare(index, j, i, content, (ev) => {dropOnSquare(ev, this)}, color, this.inTurn()))
        index++
        black = !black
      }
    ...
    for(let i = 0; i < this.state.squares[0].length; i++) {
      coords.push(this.renderCoordinate(columnLetter(i).toUpperCase(), 'horizontal'))
    }
  ...
  initialState() {
    ...
    addPawns(1, 'white'); addPawns(6, 'black');

    var addMirrored = (row, color) => {
      var piece = (C, id) => new C({
                                color: color,
                                id: id,
                                board: this
                              });
    ...
    addMirrored(0, 'white'); addMirrored(7, 'black')
