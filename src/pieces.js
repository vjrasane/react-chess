...
const diagonals = { left_up: [-1, 1], left_down: [-1, -1], right_up: [1, 1], right_down: [1, -1] }
...
function fastReverseCheck(pos, color, squares) {
  ...
  if(diagonalHits.find((h) => h.color !== color && (h.name === 'bishop' || h.name === 'queen')))
    return true;

  var cardinalHits = getDirectionHits(pos, squares, cardinals_values);
  // console.log(cardinalHit);
  if(cardinalHits.find((h) => h.color !== color && (h.name === 'rook' || h.name === 'queen')))
    return true;
...
export function getLegalMoves(pos, piece, board) {
  ...
  return moves.filter((m) => {
    var movedState = board.executeMove(toChessCoord(pos), toChessCoord(m[0]), m[1]);
...
export class Knight extends Piece {
  ...
  moves(pos, board) {
    var moves = knight_moves.map(k => [addDir(pos, k), null]).filter(m => inBounds(m[0]))
